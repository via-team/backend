...
| `SUPABASE_SERVICE_ROLE_KEY` (optional) | `config/supabase.js` | Used only for trusted route deletions after Express checks `creator_id`; bypasses RLS when child-row cleanup or route deletion needs elevated access |
